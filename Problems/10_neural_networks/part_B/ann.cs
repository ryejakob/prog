...
public class ann{ 
	...
	//for part B adding two new fields for the derivative and antiderivative of the ativation function...
	public Func<double,double> dif;
	public Func<double,double> Int;

	//constructor:
	public ann(int n1, Func<double,double> f1,Func<double,double> dif1,Func<double,double> Int1){
	N=n1;
	f=f1;
	...
	//feedforwad for the derivative:
	public double FFdif(double x1){
	double sum=0;
		for(int i=0; i<N; i++){
		double a=par[3*i];
		double b=par[3*i+1];
		double w=par[3*i+2]; //the parameters
		sum=sum+dif((x1-a)/b)*w/b; //note *w/b replace *w...
		}//end for 
	return sum; 
	}// end FFdif

	//feedforwad for anti-derivative
	public double FFint(double x1){
	double sum=0;
		for(int i=0; i<N; i++){
		double a=par[3*i];
		double b=par[3*i+1];
		double w=par[3*i+2]; //the parameters
		sum=sum+Int((x1-a)/b)*w*b; //note *w*b replace *w...
		}//end for 
	return sum; 
	}// end FFInt
