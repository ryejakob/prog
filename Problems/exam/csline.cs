...
public double cintegral(double z){
	...
	double sum=0;
	for(int n=0; n<i; n++){
		sum=sum+y1[n]*(x1[n+1]-x1[n])+b[n]/2*Pow(x1[n+1]-x1[n],2)+c[n]/3*Pow(x1[n+1]-x1[n],3)+d[n]/4*Pow(x1[n+1]-x1[n],4);
		}//end for
	return sum+y1[i]*(z-x1[i])+b[i]/2*Pow(z-x1[i],2)+c[i]/3*Pow(z-x1[i],3)+d[i]/4*Pow(z-x1[i],4);
	}//end cintegral
